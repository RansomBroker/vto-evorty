...
const WebARRocksFaceEarrings3DHelper = (function () {
    ...
    let _gl = null,
        ...
        _glShpDrawVideo = null;
    let _earLeft = null,
        _earRight = null;
    let _landmarksStabilizer = null;
    let _shpDrawVideoUniformTransform2D = null,
    ...
    const _comp = {
        // computation intermediary results:
        urCrossK: null,
        ulCrossK: null,
        denom: null,
    };
    ...
    function draw_video() {
        ...
        _gl.uniformMatrix2fv(
            _shpDrawVideoUniformTransform2D,
            false,
            _videoTransformMat2
        );
    ...
    function init_drawVideoShp() {
      ...
      void main(void){\n\
        ...
        _shpDrawVideoUniformTransform2D = _gl.getUniformLocation(
            _glShpDrawVideo,
            "transform"
        );
    ...
    window.debugThree = _three;//*/
        ...
        const viewAspectRatio =
            _spec.canvasThree.width / _spec.canvasThree.height;
        ...
        // set postprocessing:
        const renderScenePass = new THREE.RenderPass(
            _three.scene,
            _three.camera
        );
        if (_spec.taaLevel > 0) {
        ...
        // occluder material:
        _three.occluderMat = _spec.debugOccluder
    ...
    function update_earringVisibility(threeObject, wayFactor, ry) {
        ...
        const angleHide = -(
            _spec.angleHide +
            _spec.angleHysteresis * visibleFactor
        );
    ...
    function extract_earringPosition(
        earBottomLandmark,
        earEarringLandmark,
        ear
    ) {
    ...
    function callbackTrack(detectState) {
        ...
        if (detectState.isDetected) {
            ...
            let lms = _landmarksStabilizer.update(
            ...
            extract_earringPosition(
                lms[_lmIndPerLabel.rightEarBottom],
                lms[_lmIndPerLabel.rightEarEarring],
                _earRight
            );
            extract_earringPosition(
                lms[_lmIndPerLabel.leftEarBottom],
                lms[_lmIndPerLabel.leftEarEarring],
                _earLeft
            );

            // compute dr and dl, distance from camera to ears:
            const K = _headPose.ear2ear;
            const ur = _earRight.u,
                ul = _earLeft.u;
            const ulCrossK = _comp.ulCrossK,
                urCrossK = _comp.urCrossK;
        ...
        update_threeCamera: function () {
            ...
            if (canvasAspectRatio > videoAspectRatio) {
                ...
                fov =
                    (2 *
                        Math.atan(
                            (cvh / cvhs) * Math.tan(0.5 * fov * _deg2rad)
                        )) /
                    _deg2rad;
            }
            _cameraFoVY = fov;
            ...
            console.log(
                "INFO in update_threeCamera(): resolution = ",
                cvw,
                cvh
            );
        },
