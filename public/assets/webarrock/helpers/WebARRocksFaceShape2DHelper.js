...
const WebARRocksFaceShape2DHelper = (function () {
    ...
    function build_shape(landmarkLabels, shapeSpecsArg, shapeIndex) {
        ...
        return new Promise(function (accept, reject) {
            ...
            if (isIVals) {
                ...
                const GLSLType = ["float", "vec2", "vec3", "vec4"][
                    iValCCount - 1
                ];

                iValsShaderSources.vertexPars =
                    "attribute " + GLSLType + " aiVal;\n";
                iValsShaderSources.vertexPars +=
                    "varying " + GLSLType + " iVal;\n";
            ...
            // build textures:
            const texturesPromises = shapeSpecs.textures.map(function (
                textureSpec
            ) {
        ...
        void main(void){\n\
          ...
          " +
            ...
            const GLSLUniforms = shapeSpecs.uniforms.map(function (
                uniformSpec
            ) {
                ...
                return (
                    "uniform " + GLSLUniformType + " " + uniformSpec.name + ";"
                );
            });
            ...
            // link shader program:
            shp.attributes.position = _gl.getAttribLocation(
                shp.program,
                "position"
            );
            if (isIVals) {
                shp.attributes.aiVal = _gl.getAttribLocation(
                    shp.program,
                    "aiVal"
                );
            }
            ...
            if (shapeSpecs.interpolations) {
                ...
                for (let i = 0; i < interpolationsSplitted.length; ++i) {
                    ...
                    interpolatedPoints =
                        interpolatedPoints.concat(interpPoints);
            ...
            shapeSpecs.points.forEach(function (label, ind) {
                ...
                if (lmInd === -1) {
                    throw new Error(
                        "The neural network does not outputs this landmark" +
                            label
                    );
                }
            ...
            texturesPromise.then(function (textures) {
                const shapeBuilt = {
                    ...
                    frontFacing: shapeSpecs.frontFacing
                        ? shapeSpecs.frontFacing
                        : "",
                    textures: textures,
                    uniforms: uniforms,
    ...
    function compute_interpolation(pointPositions, interpolation) {
        ...
        const s0 =
            ...
            l0;
        const s1 =
            (sizeRef * interpolation.tangentInfluences[otherPointInd]) / l1;
        (m0[0] *= s0), (m0[1] *= s0);
        (m1[0] *= s1), (m1[1] *= s1);
        ...
        return new Promise(function (accept, reject) {
            const initSettings = {
                ...
                callbackReady: function (err, objs) {
                    ...
                    Promise.all(
                        _spec.shapes.map(
                            build_shape.bind(null, objs.landmarksLabels)
                        )
