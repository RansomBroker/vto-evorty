...
$(window).on('load', function () {
    ...
    $.ajax({
        ...
        success: function(response) {
                    ...
                    imagesData.forEach(function (images, index) {
                        images.forEach(function(image, currentImage) {
                            if(image.current == current && image.material == material) {
                                savedImages[material][current] = response.fullUrl + image.filepath;
                            }
                        })
                    })
                })
            ...
            function main(){
                ...
                // initial VTO mode:
                let initialModelSettings = _settings.models[_settings.initialModel];
                ...
                new THREE.GLTFLoader(threeLoadingManager).load(modelSettings.URL, function(model){
                    ...
                    data.traverse(function(child){
                        if (child instanceof  THREE.Mesh && !materials.includes(child.material)) {
                            ...
                            if (colors.length > 0 && colors[i] != undefined && colors[i].length > 1) {
                                console.log(materials[i].name);
                                console.log('total color saved '+ colors[i].length);
                                _material_tab += `
                                    <li class="nav-item" role="presentation">
                                        <a class="nav-link tab-selected" id="material-${i}-tab" data-material="${i}" data-toggle="tab" href="#material-list-${i}" role="tab">${materials[i].name}</a>
                                    </li>
                                `;
                                createCard(i)
                            }
    ...
    $(document).on('click', '.change-model' ,function() {
        $('.change-model').removeClass('model-active')
        $(this).addClass('model-active');
        let modelId = 'wrist'+$(this).data('key')
        load_model(modelId);
    })
